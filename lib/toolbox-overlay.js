...
/** @lends ToolboxOverlay */
{
  ...
  onPanelSelected: function(eventId, panelId) {
    ...
    this.selectedPanel = getPanelObject(this.chrome, this.toolbox, panelId);
 ...
 */
function getPanelObject(chrome, toolbox, panelId) {
  let panel = toolbox.getPanel(panelId);
  if (!panel) {
    return;
  }

  let context = chrome.getContext(toolbox);
  let overlay = context.getPanelOverlay(panelId);
  if (overlay) {
    return overlay;
  }
