...
const prefBranch = "extensions." + self.preferencesBranch + ".";

const { Services } = Cu.import("resource://gre/modules/Services.jsm", {});

const TraceAPI = ["dump", "sysout", "setScope", "matchesNode", "time", "timeEnd"];

// All options (even from different extension) share the same domain.
// This is so, the Tracing console displays them all at once.
const PREF_DOMAIN = "extensions.firebug";

const PREF_JPM_RUN = prefBranch + "sdk.load.command";
const PREF_JPM_VERBOSE = prefBranch + "sdk.output.logLevel";
...
try {
  ...
  // If JPM is used to run the extension with a verbose log level, enable FBTrace
  // fallback.
 ...
 *
 * @param {string} module The id of the module (just pass module.id)
 * or module object itself.
 *
...
FBTrace.get = function(moduleId) {
  ...
  if (typeof(moduleId) == "object") {
    moduleId = moduleId.uri;
  }

  let firebug = moduleId.startsWith("resource://firebug");

  // Remove URI prefix. Firebug has special treatment. There is no
  // domain name for trace logs coming from Firebug.
  let prefixURI = module.id.slice(0, -"core/trace.js".length);
  let modulePath = firebug ? moduleId.substr(prefixURI.length) : moduleId;

  // Remove the protocol. This makes the option path a bit shorter
  // and the option tree in the Tracing console looks better.
  if (modulePath.startsWith("resource://")) {
    modulePath = modulePath.substr("resource://".length);
  }
  ...
  // Append extra prefix to the option (i.e. extra level) in the
  // Options tree in the Tracing console. This will ensure that
  // all options coming from the current extension (except of Firebug)
  // will have it's own branch.
  // xxxHonza: Firebug shouldn't be an exception.
  let prefix = firebug ? "" : (self.name + "/");
  let dbgOption = ("DBG_" + prefix + modulePath).toUpperCase();
