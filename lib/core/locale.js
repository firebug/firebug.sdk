 ...
 * $STRP - localization of a string with dynamically plural forms.
 *
 * Notes:
 * 1) Name with _ in place of spaces is the key in the firebug.properties file.
 * 2) If the specified key isn't localized for particular language, both
 * methods use the part after the last dot (in the specified name) as the
 * return value.
 *
 * Examples:
 ...
 *
 * $STR("Button Label"); - search for key "Button_Label" within a *.properties
 * file. If the key doesn't exist returns "Button Label".
 *
 * $STR("net.Response Header"); - search for key "net.Response_Header".
 ...
 *
 * Search for key "net.timing.Request_Time" within the firebug.properties file.
 * Parameters are inserted at specified places (%S) in the same order as they
 * are passed. If the key doesn't exist the method returns "Request Time".
 */
...
Locale.$STRP = function(name, args, index, bundle) {
  ...
  if (prefs["useDefaultLocale"]) {
    const pluralRule = Locale.getDefaultStringBundle().
...
Locale.registerStringBundle = function(bundleURI) {
  categoryManager.addCategoryEntry("strings-" + self.id, bundleURI,
    "", false, true);

  this.stringBundle = null;

  bundleURI = getDefaultStringBundleURI(bundleURI);
  categoryManager.addCategoryEntry("default-strings-" + self.id,
...
Locale.getDefaultStringBundle = function() {
  if (!this.defaultStringBundle) {
    this.defaultStringBundle = stringBundleService.
      createExtensibleBundle("default-strings-" + self.id);
  }
