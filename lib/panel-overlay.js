...
/** @lends PanelOverlay */
{
  ...
  onReady: function(options) {
    ...
    // in onReady, so theme changes are called after the overlay
    // is fully ready.
  ...
  themeSwitched: function(eventId, win, newTheme, oldTheme) {
    ...
    if (frame && frame.contentWindow === win) {
      // See also this MDN article about proper handling errors.
      ...
      Win.loaded(win).then(() => {
        ...
        TraceError.sysout("PanelOverlay.themeSwitched; ERROR " + err, err);
      });
    }
  },

  onThemeSwitched: function(win, newTheme, oldTheme) {
    // xxxHonza: E.g. the StyleEditorOverlay.onApplyTheme is called twice
    // when the theme changes, so avoid that here (for all panels).
  ...
  observe: function(subject, topic, data) {
    if (topic == "nsPref:changed") {
      this.onDevToolsPrefChanged(data);
    }
  },
