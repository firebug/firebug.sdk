...
/** @lends TreeView */
{
  ...
  render: function() {
    ...
    /*Trace.sysout("TreeView.render; rows", {
      rows: rows,
      state: state
    });*/
    ...
    return (
      TABLE({className: "domTable", cellPadding: 0, cellSpacing: 0,
        onClick: this.onClick, ref: "domTable"},
        TBODY({}, rows)
      )
    );
  },
  ...
  componentDidMount: function() {
    Trace.sysout("TreeView.componentDidMount", this.props);

    var members = this.initMembers(this.props.data, 0);
    this.setState({data: members, searchFilter: this.props.searchFilter});

    // ReactJS doesn't properly register event listeners if
    // the environment is XUL document. Let's do that manually
    // in such case.
    var target = this.refs.domTable.getDOMNode();
    var contentType = target.ownerDocument.contentType;
    if (contentType == "application/vnd.mozilla.xul+xml") {
      target.addEventListener("click", this.onClick, false);
    }
  },
  ...
  componentWillUnmount: function() {
    ...
    var contentType = target.ownerDocument.contentType;
    if (contentType == "application/vnd.mozilla.xul+xml") {
      target.removeEventListener("click", this.onClick, false);
    }
  },
  ...
  componentWillReceiveProps: function(nextProps) {
    ...
    Trace.sysout("TreeView.componentWillReceiveProps; update: " +
      update, nextProps);
  ...
  initMembers: function(parent, level) {
    ...
    //Trace.sysout("TreeView.initMembers; " + level, parent);
  ...
  getMembers: function(object, level) {
    ...
    var getObjectProperties = this.props.getObjectProperties ||
      this.getObjectProperties.bind(this);
    var hasProperties = this.props.hasProperties ||
      this.hasProperties.bind(this);
  ...
  fetchMembers: function(object, level) {
    ...
    Trace.sysout("TreeView.fetchMembers;", children);
...
var TreeRow = React.createFactory(React.createClass({
  ...
  componentDidMount: function() {
    ...
    var target = this.refs.row.getDOMNode();
    var contentType = target.ownerDocument.contentType;
    if (contentType == "application/vnd.mozilla.xul+xml") {
      target.addEventListener("click", this.onClick, false);
    }
  ...
  render: function() {
    ...
    /*Trace.sysout("TreeRow.render; " + member.name, {
      member: member,
      value: value,
    });*/

    var TAG = Reps.getRep(value);
    return (
      TR({className: classNames.join(" "), onClick: this.onClick, ref: "row"},
