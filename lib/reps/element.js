...
/** @lends Element */
{
  ...
  getSelectorClasses: function(grip) {
    ...
    var className = preview.attributes[className];
    if (!className) {
      return "";
    }
  ...
  attrIterator: function(grip) {
    ...
    return attrs.map(attr => AttrFactory({object: attr}));
  },
  ...
  shortAttrIterator: function(grip) {
    ...
    if (grip.preview.attributes) {
      for (var name in grip.preview.attributes) {
        ...
        if (attr.localName == "id" || attr.localName == className) {
          attrs.push(attr);
        }
      }
    }

    return attrs.map(attr => AttrFactory(attr));
  },
...
const AttrFactory = React.createFactory(Attr);
