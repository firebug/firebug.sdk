...
/** @lends PanelOverlay */
{
  ...
  setupSidePanels: function() {
    ...
    Trace.sysout("PanelSidebarOverlay.setupSidePanels; " + this.sidebar, sidePanels);
  ...
  buildSidePanel: function(ctor) {
    ...
    Trace.sysout("PanelSidebarOverlay.buildSidePanel; " + id, ctor);
    ...
    if (!panelFrame) {
      TraceError.sysout("PanelSidebarOverlay.buildSidePanel; ERROR no panel frame!" + id);
      return;
    }
  ...
  onSidePanelLoad: function(event, id) {
    ...
    Trace.sysout("PanelSidebarOverlay.onSidePanelLoad; " + id, panel);
  ...
  removeSidePanels: function() {
    ...
    for (let Panel of sidePanels) {
      ...
      Trace.sysout("removeSidePanel: " + id, this.sidePanels);

      this.sidePanels.get(id).destroy();
      this.sidePanels.delete(id);

      // The method removes also the this.sidebar._tabbox.tabpanels
      // if there is no tab left. This is a problem since the
      // this.sidebar.destroy() method doesn't check value of
      // this.sidebar._tabbox.tabpanels and crashes.
