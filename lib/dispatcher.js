...
/**
 * Global dispatcher that is used to fire global events
 */
var Dispatcher = {
  ...
  once: function(type, listener) {
    ...
    this.on(type, function observer(args) {
      ...
      try {
        ...
        TraceError.sysout("Dispatcher.once; EXCEPTION " + err, err);
      }
    });
  },
  ...
  emit: function(type, args) {
    ...
    for (let i=0; i<listeners.length; i++) {
      try {
        ...
        TraceError.sysout("Dispatcher.emit; EXCEPTION " + err, err);
      }
    }
  ...
  dispatch: function(name, args) {
    ...
    for (let i=0; i<modules.length; i++) {
      ...
      try {
        ...
        TraceError.sysout("Dispatcher.dispatch; EXCEPTION " + err, err);
      }
    }
