...
/** @lends ToolboxChrome */
{
  ...
  initialize: function() {
    ...
    return Dispatcher.emit("initialize");
  },
  ...
  shutdown: function() {
    ...
    return Dispatcher.emit("shutdown");
  },
  ...
  onPrefChanged: function(eventType, data) {
    Trace.sysout("ToolboxChrome.onPrefChanged; ", data);

    if (data.pref == "devtools.theme") {
      emit(this, "theme-changed", data.newValue, data.oldValue);
    }
  },
  ...
  isFirebugThemeActive: function() {
    return this.getCurrentTheme() == "firebug";
  },
  ...
  registerPanelOverlay: function(overlay) {
    ...
    if (!panelId || !overlayId) {
      TraceError.sysout("ToolboxChrome.registerOverlay; ERROR " +
        "Missing overlay ID!");
      return;
    }
    ...
    let onApplyOverlay = (eventId, toolbox, panelFrame) => {
      ...
      // Bail out if there is already an overlay for the panel.
      if (context.getPanelOverlay(panelId)) {
        TraceError.sysout("ToolboxChrome.initialize; There is already an " +
          "overlay for: " + panelId + " (overlay: " + overlayId + ")");
        return;
      }
  ...
  getOverlay: function(toolbox, overlayId) {
    let context = this.getContext(toolbox);
    return context.getOverlay(overlayId);
  },
